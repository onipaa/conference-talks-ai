...
- 🎯 Entry points to execute your program using `python -m <conference_talks_ai>` or `$ conference_talks_ai` with basic CLI argument parsing.
...
---
# conference_talks_ai Flask Application

Awesome conference_talks_ai created by onipaa
...
```bash
git clone https://github.com/onipaa/conference-talks-ai conference_talks_ai
cd conference_talks_ai
make install
```
...
```bash
pip install conference_talks_ai
```
...
```bash
conference_talks_ai create-db   # run once
conference_talks_ai populate-db  # run once (optional)
conference_talks_ai add-user -u admin -p 1234  # ads a user
conference_talks_ai run
```
