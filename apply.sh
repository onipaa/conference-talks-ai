...
original_name="conference_talks_ai"
original_urlname="conference-talks-ai"
original_description="Awesome conference_talks_ai created by onipaa" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/conference_talks_ai "${name}"
...
then
    ...
    echo "then run 'conference_talks_ai' or 'python -m conference_talks_ai'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'conference_talks_ai' or 'python -m conference_talks_ai' or 'poetry run conference_talks_ai'"
fi
